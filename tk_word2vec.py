...
class Encoder():
  ...
  def __init__(s):
    ...
    # Input data.
    s.train_inputs = tf.placeholder(tf.int32, shape=[batch_size])
    s.train_labels = tf.placeholder(tf.int32, shape=[batch_size, 1])
    ...
    s.optimizer = tf.train.GradientDescentOptimizer(1.0).minimize(s.loss)
...
with tf.Session(graph=graph) as session:
  ...
  for step in xrange(num_steps):
    ...
    feed_dict = {enc.train_inputs: batch_inputs, enc.train_labels: batch_labels}
